...
state_serializer::deserialize( std::string const &Scenariofile ) {
    ...
    if( Scenariofile != "$.scn" ) {
        ...
        scratchpad.binary.terrain = Region->is_scene( Scenariofile ) ;
    }
    ...
    for( auto& x : simulation::Region->get_all_sections() )
    {
        ...
        for( float yy = 0.0; yy < sub_sections; ++yy )
            for( float xx = 0.0; xx < sub_sections; ++xx )
            {
                ...
                s.m_name = "none";
                s.m_data.rangesquared_min = 0;
                s.m_data.rangesquared_max = 1000'000;
                s.m_data.lighting.ambient.r = 0.025f;
                s.m_data.lighting.ambient.g = 0.025f;
                s.m_data.lighting.ambient.b = 0.025f;
                s.m_data.lighting.ambient.a = 1.0f;
            
                s.m_data.lighting.diffuse.r = 0.5f;
                s.m_data.lighting.diffuse.g = 0.5f;
                s.m_data.lighting.diffuse.b = 0.5f;
                s.m_data.lighting.diffuse.a = 1.0f;
            
                s.m_data.lighting.specular.r = 0.010f;
                s.m_data.lighting.specular.g = 0.010f;
                s.m_data.lighting.specular.b = 0.010f;
                s.m_data.lighting.specular.a = 1.0f;
            
                s.m_data.material = GfxRenderer.Fetch_Material( "grass/grass04" );
                s.m_data.translucent = false;
                ...
                s.m_data.vertices.emplace_back( v );
                v.position.x = 0.0f + origin.x + xx*size;
                ...
                s1.m_data.vertices.emplace_back( v );
...
state_serializer::deserialize_camera( cParser &Input ) {
    ...
    glm::dvec3 xyz, abc;
    int i = -1, into = -1; // ditito której definicji kamery wstawić
    std::string token;
